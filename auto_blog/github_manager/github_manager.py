...
""")
    ...
    def commit_and_push_changes(self, message: str) -> bool:
        ...
        Ensures ALL files in the github_repo directory are added and pushed.
        ...
        try:
            ...
            try:
                ...
                # Method 1: Use GitPython to add everything
                self.repo.git.add(A=True)  # Same as 'git add --all'
                
                # Method 2: As a backup, use direct git command for more reliability
                try:
                ...
                # Print a list of files that will be committed (for debugging)
                status_output = self.repo.git.status(short=True)
                if status_output:
                    logger.info(f"Files staged for commit:\n{status_output}")
                
            except Exception as add_error:
                logger.warning(f"Error adding files: {add_error}")
            ...
            if self.repo.active_branch.name != clean_branch:
                logger.info(f"Switching to branch {clean_branch} before committing")
                self.switch_branch(clean_branch)
            ...
            # Try to push changes
            try:
                logger.info(f"Pushing changes to remote branch: {clean_branch}")
                self.repo.git.push("origin", clean_branch)
                logger.info("Successfully pushed changes")
                return True
            except Exception as push_error:
                logger.warning(f"Error pushing changes: {push_error}")
                
                # Try fetching first to check for remote changes
                try:
                    logger.info("Fetching remote changes before retrying push")
                    self.repo.git.fetch("origin", clean_branch)
                    
                    # Try a rebase push approach
                    try:
                        logger.info("Attempting to rebase and push")
                        self.repo.git.pull("--rebase", "origin", clean_branch)
                        self.repo.git.push("origin", clean_branch)
                        ...
                        # Last resort: force push if allowed
                        try:
                            logger.warning("Attempting force push as last resort")
                            self.repo.git.push("-f", "origin", clean_branch)
                            logger.info("Successfully force pushed changes")
                            return True
                        except Exception as force_error:
                            logger.error(f"Force push failed: {force_error}")
                            return False
                except Exception as fetch_error:
                    logger.error(f"Error fetching remote: {fetch_error}")
                    return False
